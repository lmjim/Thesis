...
Name: Lily Jim
Updated: 11/2/2020
Description: create initial map documents from templates
"""

import arcpy # import ArcGIS functions

"""PDF ---------------------------------------------------------------------------------------------------------"""
'''Create initial documents from template'''
def setUpPDF(years, wrkspc, templateDocument, category):
    # Create each document
    for year in years:
        # Create document for new year
        #currentDataSet = "data" + str(year) # define currect data set
        untitled = arcpy.mapping.MapDocument(templateDocument) # open blank document
    ...
    for year in years:
        ...
        arcpy.Copy_management("shapeFiles/US_states.shp", wrkspc + "\\" + str(year) + "\\" + category + "\\US_states.shp")
        arcpy.AddField_management(wrkspc + "\\" + str(year) + "\\" + category + "\\US_states.shp", "CRUDE_RATE", "DOUBLE")
        usStates = arcpy.mapping.Layer(wrkspc + "\\" + str(year) + "\\" + category + "\\US_states.shp") # create layer from shapefile
        ...
        # Join table to layer
        table = "dataTables/" + category + "/" + currentDataSet + ".csv" # define data table
...
"""JPG ---------------------------------------------------------------------------------------------------------"""
'''Create initial documents from template'''
def setUpJPG(years, wrkspc, templateDocument, category):
    # Create each document
    for year in years:
        # Create document for new year
        untitled = arcpy.mapping.MapDocument(templateDocument) # open blank document
    ...
    for year in years:
        ...
        # Mainland
        df1.elementHeight = 4.1538
        df1.elementWidth = 9.0
        df1.elementPositionX = 0.0
        df1.elementPositionY = 0.0
        # Hawaii
        df2.elementHeight = 1.0
        df2.elementWidth = 1.8373
        df2.elementPositionX = 1.8372
        df2.elementPositionY = 0.0
        # Alaska
        df3.elementHeight = 1.0
        df3.elementWidth = 1.8372
        df3.elementPositionX = 0.0
        df3.elementPositionY = 0.0
